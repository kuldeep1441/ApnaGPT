...
const Summary = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const {data} = await axios.post('/api/v1/openai/summary', { text });        // ERROR PART
      // console.log(data);
      setSummary(data);
    }
    catch (err) {
  ...
  return (
    ...
    >
      ...
      {summary ?(<Card sx={{ mt: 4, border: 1, boxShadow: 0, height: '500px', borderRadius: 5, borderColor: 'natural.medium', bgcolor: 'background.default' }}>
        ...
        (<Card sx={{ mt: 4, border: 1, boxShadow: 0, height: '500px', borderRadius: 5, borderColor: 'natural.medium', bgcolor: 'background.default' }}>
          <Typography variant='h5' color="natural.main" sx={{ textAlign: 'center', verticalAlign: 'middle', lineHeight: '450px' }}>Summary will appear Here</Typography>
        </Card>)}
    </Box>
  )
}
